...
import { Category, Favourites, Post } from './post';
...
export class PostsService {
  ...
  /* ---  FUNZIONE RICHIAMO LISTA POST PER COMPONENT LISTA POST --- */
  getPostList(filter: string) {

    if (filter != "") { // Se il filtro non è vuoto eseguo funzione di filtraggio dati
      return this.getFilteredResults(filter, this.postList);
    }
    // altrimenti salto il filtraggio e mando i dati al component
    return this.postList;

  }//getPostList

  /* ---  FUNZIONE RICHIAMO LISTA PREFERITI PER COMPONENTE PREFERITI --- */
  getFavouriteList(): Favourites[] {
    return this.favourites;
  }// getFavouriteList

  /* ---  FUNZIONE RICHIAMO LISTA CATEGORIE PER COMPONENTE MENU --- */
  getCategoriesList() {
    return this.categories;
  }// getCategoriesList

  /* ---  FUNZIONE AGGIUNGI POST A PREFERITI --- */
  addToFavourites(post: Favourites) {
    console.log(post)
    let p = this.favourites.find(p => post.title == p.title);

    if (p == undefined) { // se l'elemento non è ancora inserito lo inserisco
      this.favourites.push(post)
    }

  }//addToFavourites

  /* ---  FUNZIONE RIMUOVI POST DAI PREFERITI --- */
  removeItemFromFavourites(f: Favourites) {
    let indexDelete = this.favourites.indexOf(f);
    this.favourites.splice(indexDelete, 1);
  ...
  getFilteredResults(filter: string, array: Post[]): Post[] {
